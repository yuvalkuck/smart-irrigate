...
static void nvsCreateKeyStr(const char *name, const char *defaule_value = "") {
    ...
    err = nvs_get_str(hNVS, name, NULL, &len);
...
esp_err_t init_flash(void) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "failed to open partition: %s:%s", NVS_PARTITION_NAME,NVS_NAMESPACE);
        return 0;
    }
    ...
    nvsCreateKeyStr("mqtt_client_username", CONFIG_MQTT_CLIENT_USERNAME);
    nvsCreateKeyStr("mqtt_client_password", CONFIG_MQTT_CLIENT_PASSWORD);
    nvsCreateKeyStr("ntp_server",CONFIG_NTP_SERVER);
    nvsCreateKeyStr("default_locale_time_zone",CONFIG_DEFAULT_LOCALE_TIME_ZONE);
    nvs_close(hNVS);
    return ESP_OK;
}