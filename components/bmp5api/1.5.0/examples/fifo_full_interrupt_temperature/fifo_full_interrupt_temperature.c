...
int main(void)
{
    ...
    struct bmp5_sensor_data sensor_data[BMP5_FIFO_T_FRAME_COUNT] = { { 0 } };
    ...
    if (rslt == BMP5_OK)
    {
        ...
        fifo.frame_sel = BMP5_FIFO_TEMPERATURE_DATA;
        ...
        while (loop < 5)
        {
            ...
            if (int_status & BMP5_INT_ASSERTED_FIFO_FULL)
            {
                ...
                if (rslt == BMP5_OK)
                {
                    ...
                    if (rslt == BMP5_OK)
                    {
                        printf("\n%-10s %-20s\n", "Index", "Temperature (deg C)");
                        for (idx = 0; idx < fifo.fifo_count; idx++)
                        {
#ifdef BMP5_USE_FIXED_POINT
#if defined(MCU_I2C) || defined(MCU_SPI)
                            printf("%-10u %-20lu\n", idx, (long int)sensor_data[idx].temperature);
#else
                            printf("%-10u %-20I64u\n", idx, sensor_data[idx].temperature);
#endif
#else
                            printf("%-10u %-20f\n", idx, sensor_data[idx].temperature);
#endif
